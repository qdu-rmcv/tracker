...
bool Detector::calibrate_kpts(Blade & blade, cv::Mat & img)
{
  ...
  outP.x = (blade.kpt[0].x + blade.kpt[3].x) / 2;
  outP.y = (blade.kpt[0].y + blade.kpt[3].y) / 2;
  inP.x = (blade.kpt[1].x + blade.kpt[4].x) / 2;
  inP.y = (blade.kpt[1].y + blade.kpt[4].y) / 2;
...
void Detector::draw_blade(cv::Mat & img)
{
  for (size_t i = 0; i < blade_array_.size(); ++i) {
    ...
    for (int j = 0; j < 2; j++) {
      ...
      cv::line(img, kpt_start, kpt_end, cv::Scalar(0, 255, 0), 40);
    }
    cv::circle(
      img, cv::Point(blade_array_[i].kpt[2].x, blade_array_[i].kpt[2].y), 40, cv::Scalar(255, 0, 0),
      -1);
  }
}
