...
public:
    ...
    Result Match(const T& a, const U& b)
    {
        if(Diff_us>=0)
        {
            uint64_t delay = static_cast<uint64_t>(a.time - b.time);
        ...
        Diff_us = -Diff_us;
        uint64_t delay = static_cast<uint64_t>(b.time - a.time);
